...
export default function StartInterviewPage() {
  const navigate = useNavigate();
  // In the new architecture, this 'interviewId' from the URL is actually your highly secure Python 'sessionId'
  ...
  const [candidateEmail, setCandidateEmail] = useState('');
  const [jobTitle, setJobTitle] = useState('');
  const [duration, setDuration] = useState<number>(30); // Added duration state
  const company = 'HireIQ';

  useEffect(() => {
    const fetchSessionDetails = async () => {
      try {
        // Fetch the secure session data directly from the Python backend!
        const response = await axios.get(
          `${import.meta.env.VITE_AI_API_BASE_URL || 'http://localhost:7000'}/api/interview/session/${interviewId}`
        );

        const { data } = response.data;

        // Populate the UI with the data Python returned
        setJobTitle(data.job_title || 'Applicant');
        ...
        setError(
          err.response?.data?.message || 'Failed to load secure interview session.'
        );
      } finally {
        setIsLoading(false);
      }
  ...
  const handleStart = (e: React.FormEvent) => {
    e.preventDefault();
    // Navigates to the room, carrying the sessionId in the URL
    navigate(`/interview/room/${interviewId}`);
  };
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-[#0B0F19] flex justify-center items-center">
        <div className="flex flex-col items-center gap-4">
          <span className="w-10 h-10 border-4 border-indigo-500/30 border-t-indigo-500 rounded-full animate-spin"></span>
          <p className="text-slate-400 font-medium">Loading your secure session...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#0B0F19] text-slate-300 p-4 md:p-8 selection:bg-indigo-500/30 font-class">
      <div className="max-w-6xl mx-auto">
        {/* --- Top Navigation Bar --- */}
        <div className="flex items-center justify-between mb-10 h-10">
          <div className="flex items-center justify-center rounded-lg overflow-hidden p-1">
            <img
              src="/hireiq-logo.png"
              alt="HireIQ Logo"
              className="w-30 h-30 object-contain"
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="bg-linear-to-b from-[#1C1635] to-[#121020] border border-indigo-500/20 rounded-2xl p-6 md:p-8 relative overflow-hidden">
              ...
              <form onSubmit={handleStart} className="space-y-5 relative z-10">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-400 mb-2">
                    Interview sent to <span className="text-indigo-400">*</span>
                  </label>
                  <input
                    type="email"
                    required
