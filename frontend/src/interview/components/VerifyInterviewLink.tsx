...
export default function VerifyInterviewLink() {
  ...
  useEffect(() => {
    const verifyToken = async () => {
      if (!token) {
        setStatus('error');
        setErrorMessage("No secure token was found in this link. Please check your email and try again.");
        return; 
      }

      try {
        const response = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/interviews/verify-magic-link?token=${token}`);
        ...
        setStatus('error');
        const message = error.response?.data?.message || "This interview link is invalid or has expired.";
      ...
      <div className="min-h-screen bg-gray-50 flex flex-col justify-center items-center px-4 font-class">
        <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-8 text-center border border-gray-100">
          ...
          <p className="text-gray-600 mb-8">
            ...
            For security reasons, interview links expire after a certain period or after their first use. Please contact your recruiter to request a new one.
          </p>
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col justify-center items-center px-4">
      <div className="flex flex-col items-center">
        ...
        <p className="text-lg font-medium text-gray-900">Verifying your secure link...</p>
        <p className="text-sm text-gray-500 mt-1">Please wait a moment while we set up your session.</p>
      </div>
    </div>
  );
}