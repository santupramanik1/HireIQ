...
export default function ActiveInterviewRoom() {
  ...
  const handleEndInterview = async () => {
    ...
    try {
      ...
      const totalDurationAllowed = (interviewData?.duration || 30) * 60;
      const secondsElapsed = totalDurationAllowed - timer; 
      const userResponses = transcript.filter(msg => msg.role === 'user').length; 
    ...
    <div className="h-screen bg-[#0B0F19] text-white flex flex-col overflow-hidden font-class">
      ...
      <header className="h-16 border-b border-slate-800 flex items-center justify-between px-4 md:px-6 shrink-0 bg-[#0B0F19] z-20">
        ...
        <div className="flex items-center gap-4 md:gap-6">
          ...
          {/* DYNAMIC BUTTONS WITH LOADERS */}
          {!isCallActive && !isEndingCall ? (
            <button
              onClick={handleStartInterview}
              disabled={isStartingCall}
              className={`px-4 py-2 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-500 border border-emerald-500/20 rounded-lg text-sm font-bold transition-all flex items-center gap-2 ${isStartingCall ? 'opacity-60 cursor-not-allowed' : ''}`}
            >
              {isStartingCall ? (
                <>
            ...
            <button
              ...
              className={`px-4 py-2 bg-red-500/10 hover:bg-red-500/20 text-red-500 border border-red-500/20 rounded-lg text-sm font-bold transition-all flex items-center gap-2 ${isEndingCall ? 'opacity-60 cursor-not-allowed' : ''}`}
            >
              {isEndingCall ? (
                <>
