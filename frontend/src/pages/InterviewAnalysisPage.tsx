...
import type { InterviewResult } from "./SchedulesPage";
    ...
    <div className="min-h-screen bg-slate-50/50 p-4 sm:p-6 lg:p-8 font-class">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col gap-4 mb-8">
          <button
            onClick={onBack}
            className="group flex items-center gap-2 text-slate-500 hover:text-indigo-600 font-medium transition-colors w-fit cursor-pointer"
          >
            <span className="material-symbols-outlined text-[20px] transition-transform group-hover:-translate-x-1">
              arrow_back
            </span>
          ...
          <div>
            <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">
              {data.name}'s Analysis
            </h1>
            <p className="text-slate-500 mt-1 flex items-center gap-2 text-sm sm:text-base">
              <span className="material-symbols-outlined text-[18px]">work</span>
              {data.role} <span className="mx-1">•</span> {data.dateCompleted}
            </p>
          </div>
        </div>
          ...
          <div className="lg:col-span-4 flex flex-col gap-6 lg:sticky lg:top-8">
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
              ...
              <div className="flex flex-col items-center justify-center mb-8">
                <div className="text-5xl font-black text-slate-900 tracking-tighter">
                  {data.score}
                  <span className="text-2xl text-slate-400 font-medium tracking-normal">/10</span>
                </div>
                <p className="text-sm font-medium text-slate-400 mt-1">Overall Score</p>
              </div>
              ...
              <div className="space-y-6">
                <div>
                  <div className="flex justify-between text-sm font-bold mb-2">
                    <span className="text-slate-600">Communication</span>
                    <span className="text-slate-900">{data.communicationScore}/10</span>
                  </div>
                  <div className="w-full bg-slate-100 rounded-full h-2.5 overflow-hidden">
                    <div className="bg-blue-500 h-full rounded-full transition-all" style={{ width: `${(data.communicationScore / 10) * 100}%` }}></div>
                  </div>
                </div>
                ...
                <div>
                  <div className="flex justify-between text-sm font-bold mb-2">
                    <span className="text-slate-600">Technical</span>
                    <span className="text-slate-900">{data.technicalScore}/10</span>
                  </div>
                  <div className="w-full bg-slate-100 rounded-full h-2.5 overflow-hidden">
                    <div className="bg-orange-500 h-full rounded-full transition-all" style={{ width: `${(data.technicalScore / 10) * 100}%` }}></div>
                  </div>
                </div>
                ...
                <div>
                  <div className="flex justify-between text-sm font-bold mb-2">
                    ...
                    <span className="text-slate-900">{data.confidenceScore}/10</span>
                  </div>
                  <div className="w-full bg-slate-100 rounded-full h-2.5 overflow-hidden">
                    <div className="bg-purple-500 h-full rounded-full transition-all" style={{ width: `${(data.confidenceScore / 10) * 100}%` }}></div>
                  </div>
                </div>
              </div>
          ...
          <div className="lg:col-span-8 flex flex-col gap-6">
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 sm:p-8">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="font-bold text-slate-800 text-lg">AI Analysis</h2>
              </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
              <div className="p-4 sm:p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/30">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="font-bold text-slate-800 text-lg">Full Conversation Transcript</h2>
                </div>
                <span className="text-xs font-bold text-slate-400 bg-slate-100 px-3 py-1 rounded-full">
                  {data.transcript?.length || 0} messages
                </span>
              </div>
                ...
                {data.transcript && data.transcript.length > 0 ? (
                  data.transcript.map((msg, index) => (
                    <div key={index} className="flex items-start gap-4">
                      ...
                      >
                        {msg.role === 'ai' || msg.role === 'assistant' ? 'AI' : data.initials}
                      </div>

                      <div className="flex-1 min-w-0 pt-1">
                        <div className="flex items-baseline gap-2 mb-1">
                          <span className="font-bold text-slate-900 text-sm">
                            {msg.role === 'ai' || msg.role === 'assistant' ? 'AI Interviewer' : data.name}
                          </span>
                        </div>
                        <p className="text-slate-600 text-sm sm:text-[15px] leading-relaxed">
                          {msg.text}
                  ...
                  <p className="text-center text-slate-400 italic">Transcript unavailable.</p>
                )}
              </div>
            </div>
          </div>
