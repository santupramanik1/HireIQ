          ...
          const formattedCandidates = data.data.candidates.map(
            (backendData: any, index: number) => {
              ...
              return {
                ...
                candidateId: backendData.candidateId, // Catch this
                applicationId: backendData.applicationId, // Catch this
                initials: initials,
  ...
  const handleSendInvites = async (interviewType: string) => {
    ...
    try {
      for (const email of selectedIds) {
        ...
        // Call your NEW interview creation route, not just the email route
        await axios.post(
          `${import.meta.env.VITE_API_BASE_URL}/interviews/invite`, // Ensure you have this route set up in Express!
          {
            jobId: jobId,
            applicationId: candidate.applicationId,
            candidateId: candidate.candidateId,
            // recruiterId should ideally be extracted from req.user (JWT) in the backend controller
